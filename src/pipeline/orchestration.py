...
from pipeline.point_generation import (
    Transmitter,
...
from utils.logging import Timer, ProgressTracker, print_success, print_warning
...
class PipelineOrchestrator:
        ...
        if HAS_SRTM:
            try:
                with Timer("Initialize SRTM elevation data"):
                    # Set cache directory to project SRTM cache
                    set_srtm_cache_dir(str(paths['srtm_dir']))
    ...
    def run_phase1_dataprep(
        ...
        if not landcover_cache_dir:
            landcover_cache_dir = self.phase0_paths['landcover_dir']
        ...
        from pipeline.data_preparation import LandCoverProcessor
    ...
    def run_phase3_extraction(self, dem_path: Optional[Path] = None) -> gpd.GeoDataFrame:
        ...
        if not landcover_path:
            ...
            landcover_path = self.phase0_paths['landcover_dir'] / (
                f"lcm10_{lat}_{lon}_{year}_buf{buffer_m}m_{chip_px}px.tif"
            )
        
        # Get zones path
        zones_path = self.phase0_paths['brzones_dir'] / 'zones_map_BR.json'
