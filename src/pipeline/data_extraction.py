...
from utils.logging import Timer, print_success, print_warning, print_error
from utils.validation import ValidationError, validate_geodataframe
...
class RasterPreloader:
    ...
    def _extract_elevation_srtm(self, gdf: gpd.GeoDataFrame) -> np.ndarray:
        ...
        """
        try:
            from propagation.profile_extraction import _get_srtm_data
            elevation = np.zeros(len(gdf), dtype=np.float32)
