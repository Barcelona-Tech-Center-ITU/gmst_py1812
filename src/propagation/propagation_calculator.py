...
def _generate_smart_filename(results, input_csv_path):
    """
    Generate smart output filename matching input CSV metadata.
    
    Format: results_{TX_ID}_{PROFILES}p_{AZIMUTHS}az_{DISTANCE}km_v{TIMESTAMP}_{HASH}.csv
    """
    if not results:
        return None
    ...
    filename = f"results_{tx_id}_{profiles_count}_{azimuths}_{distance}_{version_hash}.csv"
...
def _save_results(results, input_csv_path, output_dir):
    """
    Save P1812 results to CSV with smart naming.
    ...
    csv_filename = _generate_smart_filename(results, input_csv_path)
...
def main(profiles_dir=None, output_dir=None):
    """Main batch processor function with smart file naming.
    
    Loads profiles from CSV and calculates P1812 propagation loss/field strength.
    Results are saved with smart naming matching input CSV metadata.
    
    Parameters:
    -----------
    profiles_dir : Path or str, optional
        Directory containing profile CSV files. Defaults to data/profiles/
    output_dir : Path or str, optional
        Output directory for results. Defaults to data/output/
    ...
    # __file__ -> /path/to/src/propagation/batch_processor_v2.py
    # .parent -> /path/to/src/propagation/
    # .parent -> /path/to/src/
    ...
    csv_path = _save_results(results, input_csv_path, output_dir)
    
    print(f"\n✓ Saved CSV: {csv_path.name}")
    print(f"  Size: {csv_path.stat().st_size / 1024:.1f} KB")
    
    print(f"\n📊 Filename format: results_{{TX_ID}}_{{PROFILES}}p_{{AZIMUTHS}}az_{{DISTANCE}}km_v{{TIMESTAMP}}_{{HASH}}.csv")
    
    print(f"\n{'='*70}")
    print(f"✅ PROPAGATION CALCULATIONS COMPLETE")
    print(f"{'='*70}")
